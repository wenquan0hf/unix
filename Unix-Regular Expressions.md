...
正则表达式是一个字符串,可以用来描述几个字符序列。正则表达式是使用几个不同的Unix命令,包括 ed、sed、awk、grep,以及 vi。
...
在这里,模式是一个正则表达式,action则是在下表中给出的命令。当省略模式时,如上面我们已经看到的，action 会执行每一行命令。


围绕模式的斜杠字符(/)是必需的,因为它们是作为分隔符使用。
...
再次调用 sed,但这一次告诉 sed 删除命令行,用单个字母d表示:
...
sed 也理解所谓的地址。地址可以是文件中的一个位置，也可以是一个特殊的编辑命令适用的范围。当 sed 遇到没有地址的情况时,它会对文件中的每一行执行其操作。


下面的命令将一个基本的地址添加到您已经使用的 sed 命令:
...
用一个字符串替代另一个,你需要告诉 sed 你第一个字符串和想要替换的字符串的结束和开始。传统上是由正斜杠(/)将两个字符串分开的。
...
来看看在其他的表达式里面是如何演示元字符的使用。例如下面的模式:
...
一些特殊的关键字对regexp来说是容易获得的,尤其是GNU实用程序会使用regexp。对 sed正则表达式来说这些都是非常有用的，因为简化了表达式并且增强了可读性。
...
例如,假设您有一个名为phone.txt.的文件，里面都电话号码,如下所示:
